...
export function SimpleRegisterForm() {
  ...
  return (
    <Card className="pt-24 pb-16 flex mx-auto items-center" color="transparent" shadow={false}>
      ...
      <form onSubmit={handleSubmit(handleSignup)} className="mb-2">
        <div className="mb-1 grid md:grid-cols-2 sm:grid-cols-1 gap-5 max-w-lg">
          {/* Name */}
          <div className="">
            <Input label="Name" placeholder="abdullah" {...register("name", { required: "Please enter your Name!" })} icon={<CiUser />} />
          </div>

          {/* Email */}
          <div className="">
            <Input label="Email" placeholder="name@mail.com" {...register("email", { required: "Please enter your Email!" })} icon={<AiOutlineMail />} />
          </div>

          {/* Role */}
          <div className="">
            <Controller name="role" control={control} rules={{ required: "Please select your Role!" }} render={({ field }) => (
          ...
          {/* Bank Account Number */}
          <div className="">
            <Input label="Bank Account Number" placeholder="1234567890" {...register("bank_account_no", { required: "Please enter your Bank Account Number!" })} icon={<MdAccountBalance />}
            />
          </div>

          {/* Salary */}
          <div className="">
            <Input label="Salary" placeholder="123456" {...register("salary", { required: "Please enter your Salary!" })} icon={<MdAttachMoney />} />
          </div>

          {/* Image */}
         <div className="">
          <Input label="Image" type="file" accept="image/*" {...register("image", { required: "Please upload your Profile Image!" })} icon={<CiImageOn />} />
         </div>

          {/* Password */}
          <div className="">
            <Input label="Password" placeholder="******" type="password" {...register("password", { required: true, minLength: 6, maxLength: 15, pattern: /(?=.*[A-Z])(?=.*[!@#$&*])(?=.*[0-9])(?=.*[a-z])/ })} icon={<TbLockPassword />} />
          </div>
        </div>
